...
def create_deployment():
    ...
    # Log the deployment
    print(f"🚀 Deployment created: {deployment_id} for {deployment['application']}")
...
```
**⚠️ Important Rules for Usernames:**
...
async def handle_numbers(message: Message):
    ...
    squared = number ** 2
    await message.answer(f"🔢 {number}² = {squared}")
...
async def handle_any_text(message: Message):
    ...
    if message.text:
        ...
        await message.answer("📝 Please send a text message or use /help")
```
...
async def send_photo_command(message: Message):
    ...
    await message.answer_photo(
        ...
        caption="🖼️ Here's your random photo!\nUse /help for more commands"
    )
...
async def handle_received_photo(message: Message):
    ...
    await message.answer("📸 Nice photo! Thanks for sharing.")
...
def get_main_keyboard():
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="🎲 Roll Dice", callback_data="dice_roll")],
            [InlineKeyboardButton(text="ℹ️ Get Info", callback_data="user_info")],
...
async def process_age(message: Message, state: FSMContext):
    ...
    if not (1 <= age <= 120):
        await message.answer("❌ Please enter a realistic age (1-120):")
        return
    
    await state.update_data(age=age)
    await state.set_state(UserProfile.waiting_for_city)
    await message.answer("🏙️ What city do you live in?")
...
async def handle_search_no_query(message: Message):
    ...
    await message.answer("❌ Please provide a search query: /search <query>")
...
async def handle_goodbyes(message: Message):
    ...
    responses = ["👋 Goodbye!", "See you later!", "Bye! Come back soon!"]
    await message.answer(random.choice(responses))
```
...
async def start_command(message: types.Message):
    await message.answer("👋 Hello! I’m your bot. Ready to work!")
```
...
async def start_webhook():
    ...
    logging.info(f"🚀 Webhook server started at http://{WEBAPP_HOST}:{WEBAPP_PORT}")
    await site.start()
